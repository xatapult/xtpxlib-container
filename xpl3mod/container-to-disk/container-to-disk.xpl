...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" xmlns:c="http://www.w3.org/ns/xproc-step"
  ...
  <p:documentation>
    Writes an xtpxlib (3) container structure to disk. A base path must be provided, either as option 
    `$href-target` path or as `/*/@href-target`.
  </p:documentation>
  ...
  <p:declare-step type="local:delete-directory" name="local-delete-directory">
    ...
    <p:output port="result" primary="true" sequence="true" content-types="any" pipe="source@local-delete-directory"/>
    ...
    <p:if test="$remove-target">
      <p:variable name="href-target-path" as="xs:string" select="/*/@_href-target-path"
        pipe="container@local-delete-directory"/>
      ...
      <p:variable name="href-target-path-no-protocol" as="xs:string"
        select="substring-after($href-target-path, 'file:///')"/>
  ...
  <p:for-each>
    ...
    <p:try>
      ...
      <p:catch>
        <xtlcon:report-error error-code="xtlcon:error-write"
          href-target="{p:document-property(., 'href-target-original')}"
